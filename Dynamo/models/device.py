...
class Device:
    ...
    def invoke_service(self, user_id: int = 0, timeout=10):
        ...
        print("INVOKE_SERVICE CMD:", cmd)
        print("INVOKE_SERVICE OUT:", out)
        if "Failure calling service activity: Broken pipe (32)" in out:
            while "Failure calling service activity: Broken pipe (32)" in out:
                out, _ = self.adb_connection.shell(cmd, timeout=timeout)
                print("INVOKE_SERVICE CMD:", cmd)
                print("INVOKE_SERVICE OUT:", out)
